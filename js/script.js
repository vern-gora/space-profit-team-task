...
function showForm() {
  $('.iframe-form').addClass('show'), $('.btn-top').addClass('show'), $('.reviews').removeClass('hide');
}
  ...
  phoneInput.addEventListener('input', function (event) {
    ...
    for (let i = 1; i < inputValue.length; i++) {
      if (!/[0-9]/.test(inputValue[i])) {
        input.value = inputValue.slice(0, i) + inputValue.slice(i + 1);
        return;
      }
    }
  });
